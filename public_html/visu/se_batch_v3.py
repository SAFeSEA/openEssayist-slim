...
import networkx as nx  # For drawing the diagrams.
import matplotlib.pyplot as plt # For drawing the diagrams.
...
for essay_fname in filelist: # For each file in the current directory...
    ...
    if essay_fname[-3:] == 'txt': # If a file name ends in 'txt'...
        ...
        print '\nEssay file name: ', essay_fname # Print to shell to monitor progress
...
#        essay, gr_se_sample, gr_ke_sample = top_level_procedure(essay_txt,essay_fname,nf,nf2,dev,"H810","TMA01") # xxxx 'essay' argument not needed here, only running EssayAnalyser not OpenEssayist, next line down is normal version
        ...
        #nx.draw(gr_se_sample, font_size=5, font_color='b', font_weight='normal',\
                ...
                #cmap=plt.cm.gist_rainbow, # Beginning is red, end is violet
                cmap=plt.cm.rainbow, # Beginning is violet, end is red
...
##        # main original design but in blues
##        string = essay_fname[:-4] + '_gr_se_sample_nodes_2' + '.png'
##        figurefilename = os.path.join(tempdir1, string)
##        #pos=nx.circular_layout(gr_se_sample)
##        #pos=nx.graphviz_layout(gr_se_sample,prog="neato")
##        plt.figure(1, figsize=(8,8))
...
##        ## Key sentence graphics for John's CAA paper
##        string = essay_fname[:-4] + '_gr_se_sample_nodes_none' + '.png'
##        figurefilename = os.path.join(tempdir1, string)
##        #pos=nx.circular_layout(gr_se_sample)
##        #pos=nx.graphviz_layout(gr_se_sample,prog="neato")
...
####        print '\n\n\n', ax
####        a = len(x) 
####        ax.plot(a-(a-1), zorder=a)# lw=10)
####        ax.plot(a-(a-2), zorder=a-1)
####        ax.plot(a-(a-3), zorder=a-2)
####        ax.plot(a-(a-4), zorder=a-3)
####        ax.plot(a-(a-5), zorder=a-4)
####        ax.plot(a-(a-6), zorder=a-5)
####        ax.plot(a-(a-7), zorder=a-6)
####        ax.plot(a-(a-8), zorder=a-7)
##        #print gr_se_sample.nodes(data = True)
##        #plt.title(essay_fname)
##        #nx.draw(gr_se_sample, font_size=0, font_color='c', font_weight='normal',\
##        nx.draw(gr_se_sample, font_size=18, font_color='b', font_weight='normal',\
##                node_size=600,
##                #stretch_factor=100,
##                #node_size=gr_se_sample.degree().values(), # node size proportional to node degree
##                alpha = 0.25, # transparency of the node               
...
##                #node_color='c',
##                node_color=range(len(x)),
##                #cmap=plt.cm.ocean, # Beginning is green, then blue, end is white
##                cmap=plt.cm.rainbow, # Beginning is violet, end is red
##                #cmap=plt.cm.gist_rainbow, # Beginning is red, end is violet
##                edge_color='k',width=.6,style='solid',arrows=False) # figure which clusters connected nodes very closely and unconnected nodes a long way away. Not sure why.
##                #edge_color='k',width=.1,style='solid',arrows=False) # figure which clusters connected nodes very closely and unconnected nodes a long way away. Not sure why.        ##plt.show() # Use this line if you want to print to a pop-up window from the shell
##        plt.savefig(figurefilename)#, dpi=1000)
##        plt.close() # If you don't close the current figure, the data from the current figure will leak into the next essay's figure.


##        ## Key sentence craphics for Denise's rainbow exercise
##        string = essay_fname[:-4] + '_gr_se_sample_nodes_none' + '.png'
##        figurefilename = os.path.join(tempdir1, string)
##        #pos=nx.circular_layout(gr_se_sample)
##        #pos=nx.graphviz_layout(gr_se_sample,prog="neato")
...
##        #plt.title(essay_fname)
##        #nx.draw(gr_se_sample, font_size=0, font_color='c', font_weight='normal',\
##        nx.draw(gr_se_sample, font_size=9, font_color='b', font_weight='normal',\
##                node_size=600,
##                #stretch_factor=100,
##                #node_size=gr_se_sample.degree().values(), # node size proportional to node degree
##                alpha = 0.25, # transparency of the node               
...
##                cmap=plt.cm.rainbow, # Beginning is violet, end is red
##                #cmap=plt.cm.gist_rainbow, # Beginning is red, end is violet
##                edge_color='k',width=.4,style='solid',arrows=False) # figure which clusters connected nodes very closely and unconnected nodes a long way away. Not sure why.
##                #edge_color='k',width=.1,style='solid',arrows=False) # figure which clusters connected nodes very closely and unconnected nodes a long way away. Not sure why.        ##plt.show() # Use this line if you want to print to a pop-up window from the shell
##        plt.savefig(figurefilename)#, dpi=1000)
##        plt.close() # If you don't close the current figure, the data from the current figure will leak into the next essay's figure.
...
##        ## Aesop key words for John CAA paper
##        string = essay_fname[:-4] + '_gr_ke_sample' + '.png' # can use pdf but don't specify dpi in plt.savefig
##        figurefilename = os.path.join(tempdir1, string)
##        pos=nx.circular_layout(gr_ke_sample)
##        plt.figure(2,figsize=(12,8))        
##        nx.draw(gr_ke_sample, pos, font_size=24, font_color='b',\
##                node_size=800,
##                #node_size=gr_se_sample.degree().values(),
##                alpha = 0.5, # transparency
##                linewidths=.001,node_shape='o', node_color='w', \
##                edge_color='k',width=1,style='solid',arrows=False) # circular figure in which the edges cross the middle like spokes            #nx.draw(gr_ke_sample) # figure which clusters connected nodes very closely and unconnected nodes a long way away. Not sure why.
##        ##plt.show() # Use this line if you want to print to a pop-up window from the shell
##        plt.savefig(figurefilename)#,dpi=1000) #
##        plt.close() # If you don't close the current figure, the data from the current figure will leak into the next essay's figure.        
...
##        plt.close() # If you don't close the current figure, the data from the current figure will leak into the next essay's figure.        
        ...
        nf2.write('\n')
        nx.write_weighted_edgelist(gr_se_sample, nf2, comments="#", delimiter=' ', encoding='utf-8') # This writes edge list to temp dir instead of to cwd
        #print '\n\n', essay, '\n\n', essay_id , '\n\n'
        nf2.write('\n')
        nf2.write('\n')
        #print gr_se_sample.nodes(data = True)
